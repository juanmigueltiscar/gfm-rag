#!/bin/bash
# Stage 2: Fine-tune GFM-RAG on master_ceramica dataset (single GPU).
# Run from the repository root: bash scripts/gfm-rag-ceramics/stage2_finetune.sh
#
# Pretrained weights are loaded with strict=False: compatible GNN layers
# initialize from rmanluo/GFM-RAG-8M; the input projection layer (shape
# changes from 512->512 to 2048->512) is randomly initialized and learned.
...
# ─── GNN architecture ────────────────────────────────────────────────────────
N_LAYERS_DIM=512             # GFM-RAG default hidden dims
...
BATCH_SIZE=4
CHECKPOINT=null
PRETRAINED_WEIGHTS="rmanluo/GFM-RAG-8M"
SAVE_BEST_ONLY=true
SAVE_PRETRAINED=true
USE_WANDB=false
RUN_NAME="stage2-finetune-gfm-rag-ceramics"
...
echo "Fine-tuning GFM-RAG on: ${TRAIN_DATA_NAME}"
